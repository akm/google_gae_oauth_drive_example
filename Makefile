...
# When you got "You do not have permission to modify this app" with 403
#
# 1. Backup your ~/.appcfg_oauth2_tokens
# 2. appcfg.py -A $PROJECT --noauth_local_webserver update ./app/google-gae-oauth-drive-example
# 3. Follow the instruction shown

GITHUB_ORG  = akm
GITHUB_REPO = google_gae_oauth_drive_example
PACKAGE  = google-gae-oauth-drive-example
...
run: fmt vendor | $(BASE) ; $(info $(M) Running dev server…) @ ## Running dev_appserver
	$Q dev_appserver.py $(CURDIR)/app/$(PACKAGE)/app.yaml


# Tests

TEST_TARGETS := test-pages
...
.PHONY: deploy
deploy: build
	appcfg.py -A $${PROJECT} -V ${VERSION} update ./app/$(PACKAGE)

.PHONY: update-traffic
update-traffic:
	gcloud --project ${PROJECT} app services set-traffic $(PACKAGE) --splits=${VERSION}=1 -q
